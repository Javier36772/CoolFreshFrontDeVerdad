...
export function Sidebar({ sidebarOpen, setSidebarOpen, onLogout }) {
  ...
  return (
    <Container isOpen={sidebarOpen} themeUse={theme}>
      ...
      {secondarylinksArray.map(({ icon, label }) => (
        <div className="LinkContainer" key={label}>
          <NavLink
            to={label === "Salir" ? "/" : "/null"} // Redirige a "/" para Salir
            onClick={label === "Salir" ? handleLogoutClick : undefined} // Llama a handleLogoutClick solo si el label es "Salir"
            className={({ isActive }) => `Links${isActive ? ` active` : ``}`}
          >
            <div className="Linkicon">{icon}</div>
            {sidebarOpen && <span>{label}</span>}
...
const linksArray = [
  ...
  { label: "Estadisticas", icon: <MdOutlineAnalytics />, to: "/estadisticas" },
...
const secondarylinksArray = [
  { label: "Configuración", icon: <AiOutlineSetting />, to: "/null" },
  { label: "Salir", icon: <MdLogout />, to: "/" }, // Aquí está el enlace de "Salir"
];
